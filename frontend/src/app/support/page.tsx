...
export default function SupportGuestPage() {
    ...
    const handleAccessTicket = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError('Invalid token or ticket not found');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="w-full max-w-2xl relative">
                ...
                {mode === 'SELECT' && (
                    <div className="glass p-10 rounded-3xl relative overflow-hidden animate-in zoom-in-95">
                        ...
                        <div className="text-center mb-10">
                            <h1 className="text-4xl font-black font-display tracking-tight mb-4">Support Hub</h1>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                            ...
                            >
                                ...
                                <div className="text-center">
                                    <h3 className="font-bold text-lg mb-1">Open a New Ticket</h3>
                                    <p className="text-xs text-slate-400 font-medium leading-relaxed">I have a new issue or question that needs attention.</p>
                                </div>
                            </button>
                            ...
                            >
                                ...
                                <div className="text-center">
                                    <h3 className="font-bold text-lg mb-1">Check Existing Ticket</h3>
                                    <p className="text-xs text-slate-400 font-medium leading-relaxed">I have my secure token and want to view replies.</p>
                                </div>
                            </button>
                        </div>
                    </div>
                ...
                {mode === 'CREATE' && (
                    <div className="glass p-10 rounded-3xl relative overflow-hidden animate-in slide-in-from-bottom-4">
                        ...
                        <h2 className="text-3xl font-black font-display tracking-tight mb-2">Create Ticket</h2>
                        ...
                        <form onSubmit={handleCreateTicket} className="space-y-5">
                            ...
                            <div>
                                <label className="block text-xs font-black uppercase tracking-widest text-slate-500 mb-2 ml-1">Issue Description</label>
                                <input 
                                    type="text" 
                                    required 
                                    className="input-field" 
                            ...
                            <button type="submit" disabled={loading} className="btn-primary w-full py-4 mt-4 flex items-center justify-center gap-2 disabled:opacity-50">
                                {loading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <>Submit Ticket <ArrowRight className="w-4 h-4" /></>}
                            </button>
                        </form>
                    </div>
                )}
                    ...
                    <div className="glass p-10 rounded-3xl relative overflow-hidden animate-in slide-in-from-bottom-4 text-center">
                        {activeTicket && !error ? (
                            <div className="animate-in zoom-in-95">
                                ...
                                <h2 className="text-3xl font-black font-display tracking-tight mb-2 text-white">Ticket Created Successfully</h2>
                                <p className="text-slate-400 font-medium mb-8 max-w-sm mx-auto">Please save your secure access token. You will need it to view replies to this ticket.</p>
                                
                                <div className="bg-black/50 border border-white/10 rounded-2xl p-6 mb-8 relative group">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 absolute -top-2 bg-[var(--background)] px-2 left-6">Access Token</p>
                                    <span className="font-mono text-xl text-[var(--accent)] font-bold select-all">{token}</span>
                                </div>

                                <button onClick={() => setMode('CHAT')} className="btn-primary px-8 py-3">Enter Chat Now</button>
                            </div>
                        ) : (
                            <div>
                                <button onClick={() => setMode('SELECT')} className="text-xs font-bold text-slate-500 uppercase tracking-widest hover:text-white transition-colors mb-8 flex items-center gap-2">
                                ...
                                <h2 className="text-3xl font-black font-display tracking-tight mb-2 text-left">Access Ticket</h2>
                                <p className="text-slate-400 font-medium mb-8 text-left">Enter your secure access token to view the conversation.</p>
                                ...
                                <form onSubmit={handleAccessTicket} className="space-y-5 text-left">
                                    <div>
                                        <label className="block text-xs font-black uppercase tracking-widest text-slate-500 mb-2 ml-1">Secure Token</label>
                                        <input 
                                            type="text" 
                                            required 
                                    ...
                                    <button type="submit" disabled={loading} className="btn-primary w-full py-4 mt-4 flex items-center justify-center gap-2 disabled:opacity-50">
                                        {loading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <>Access Chat <ArrowRight className="w-4 h-4" /></>}
                                    </button>
                                </form>
                            </div>
                        )}
                ...
                {mode === 'CHAT' && activeTicket && (
                    <div className="glass rounded-3xl relative overflow-hidden animate-in slide-in-from-bottom-4 h-[80vh] flex flex-col border-white/10 shadow-2xl">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
                             {messages.length === 0 && (
                                <p className="text-center text-slate-500 text-sm mt-10">We have received your ticket and will reply shortly.</p>
                            )}
